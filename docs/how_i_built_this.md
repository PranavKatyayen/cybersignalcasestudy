...
I ran this as the architect and reviewer: I set the direction, requirements and constraints, and worked with Claude Code (the desktop app) as my implementation partner. It wrote most of the Python, SQL and TypeScript from my prompts, and I used ChatGPT early on to cross-check the plan. The loop: understand the real data first, decide the design, have a small piece built, run it against real data, review the actual output, correct, repeat. Nothing counted as done until I had seen it working on the deployed app and in the traces.

## What I owned

- **Design constraints.** Real models only, no mock output; free tiers only; the 30,000-record sample as the final dataset; a chat assistant; prompt v1 versus v2; and the full request text in every trace.
...
- **Reading the raw data.** A sample showed a single scan snapshot, which ruled out any trend feature early.
...
- **Repository and deployment plumbing:** Vercel created a second repository and early commits carried the wrong identity; I caught both and had them corrected.
